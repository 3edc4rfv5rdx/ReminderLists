...
// (TZ 8, no long-press on records). Swipe right = rename, swipe left = delete (TZ 8 §735).
// No counter when empty, bold when non-empty.
@Composable
fun FolderRow(
    name: String,
    comment: String?,
    ...
    SwipeActionsRow(onEdit = onRename, onDelete = onDelete) {
        ListItem(
            headlineContent = {
                RowTitle(
                    name = name,
